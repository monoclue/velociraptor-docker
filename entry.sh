...
echo "mkdir done" && \
echo '{"Frontend":{"public_path":"'$PUBLIC_PATH'", "hostname":"'$VELOX_FRONTEND_HOSTNAME'"}, "API":{"bind_address":"'$BIND_ADDRESS'"}, "GUI":{"bind_address":"'$BIND_ADDRESS'"}, "Monitoring":{"bind_address":"'$BIND_ADDRESS'"}, "Logging":{"output_directory":"'$LOG_DIR'", "separate_logs_per_component":"'$LOG_PER_COMPONENT'"}, "Client":{"server_urls":["'$VELOX_SERVER_URL'"], "use_self_signed_ssl":"'$SELF_SIGNED_CERT'"}, "Datastore":{"location":"'$DATASTORE_LOCATION'", "filestore_directory":"'$FILESTORE_DIRECTORY'"}}' && \
...
echo "merge done" && \
./velociraptor --config server.config.yaml user add $VELOX_USER $VELOX_PASSWORD --role administrator &&\
echo "added user" && \
